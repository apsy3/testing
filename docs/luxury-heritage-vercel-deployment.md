...
3. Start dev server: `yarn -w apps/web dev`.
...
> **CLI tip:** When running `vercel build` from your workstation, override the
> install and build commands explicitly:
> ```bash
> vercel build \
...
> ```
> The log output should reference the vendored Yarn binary rather than `npm ci`.
