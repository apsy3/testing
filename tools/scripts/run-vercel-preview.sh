...
if [ -f "$ROOT_DIR/tools/scripts/ensure-yarn.mjs" ]; then
  node "$ROOT_DIR/tools/scripts/ensure-yarn.mjs"
fi

if [ -f "$ROOT_DIR/tools/scripts/check-yarn.sh" ]; then
  (cd "$ROOT_DIR" && bash tools/scripts/check-yarn.sh)
fi
